            ...
            <table class="table table-striped" style="table-layout: fixed;word-wrap:break-word;" id="dt">
               ...
               {# {% for endpoint in endpoints %}
                        ...
                        {% if endpoint.active == true %}
                            <tr>
                            <td><button class="btn btn-info btn-xs copy" id="copy" data-eval="{{ request.contextPath }}/ldap/{{ endpoint.endPointName }}">Copy</button></td>
                        ...
                        {% else %}
                            <tr style="color:#999">
                            <td><button class="btn btn-info btn-xs copy" id="copy1" disabled data-eval="{{ request.contextPath }}/ldap/{{ endpoint.endPointName }}">Copy</button></td>
                            <td>{{ endpoint.endPointName }}</td>
                            <td>{{ endpoint.base }}</td>
                            <td>{{ endpoint.query }}</td>
                            <td>{{ endpoint.ldapAccounts.connName }}</td>
...
0
{% block script %}
    <link type="text/javascript" rel="stylesheet" href="{{ request.contextPath }}/static/css/dt.css"/>
    <script type="text/javascript" src="{{ request.contextPath }}/static/js/dt.js"></script>
    <script type="text/javascript" src="{{ request.contextPath }}/static/js/custom/ldap_endpoint_query.js"></script>
{% endblock %}