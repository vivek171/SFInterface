$(document).ready(function () {
    ...
    var table = $('#dt').DataTable({
        ...
        "length": leng,// here
        "ajax": CONTEXT_PATH + "/jdbc/jdbcendpoints",
        headers: headers,
        "columns": [
            {"data": "", "width": "4%"},
            {"data": "endpointName", "width": "20%"},
            {"data": "query", "width": "56%"},
            {"data": "jdbcAccounts.environment", "width": "12%"},
            {"data": "active", "width": "8%"}
        ],
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, row) {
                    return row.active == true ? '<input type="button" class="btn btn-info btn-xs copy" id="copy" data-eval="'+CONTEXT_PATH +'/jdbc/runquery/'+row.endpointName+'" value="Copy">' : '<input type="button" class="btn btn-info btn-xs copy" id="copy1" disabled data-eval="'+CONTEXT_PATH+'/jdbc/runquery/'+row.endpointName+'" value="Copy">';
                }
            },
            {
                targets: [4],
                render: function (data, type, row) {
                    return row.active == true ? '<a class="btn btn-info btn-xs Edit" style="width:50" href="'+CONTEXT_PATH+'/jdbc/editep/'+row.id+'">Edit</a><a class="btn btn-danger btn-xs Deactive" style="width:50;vertical-align:bottom"  href="'+CONTEXT_PATH+'/jdbc/deactiveep/'+row.id+'"> Deactive </a>' : '<a class="btn btn-info btn-xs" style="width:50" href="'+CONTEXT_PATH+'/jdbc/editep/'+row.id+'">Edit</a><a class="btn btn-info btn-xs" style="width:50;vertical-align:bottom" href="'+CONTEXT_PATH+'/jdbc/activeep/'+row.id+'"> Active </a>';
                }
            }
        ]
    });
    ...
    $('#dt_wrapper').prepend('<a href="' + CONTEXT_PATH + '/jdbc/query" class="btn" style="margin-right:13;background:#0ea3c4 ;width:100;float: right;color:#ffffff;margin-left: 10px;">New</a>')
